...
func (r *PostgresInvitationRepository) UpdateStatus(ctx context.Context, id uint64, status string) error {
	query := `UPDATE invitation SET status = $2 WHERE id = $1`
	_, err := r.db.Exec(ctx, query, id, status)
	return err
}

func (r *PostgresInvitationRepository) AcceptInvite(ctx context.Context, userID uint64, code string) error {
